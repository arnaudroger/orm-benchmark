...
function runJmh() {
    ...
    benchmarkname=$1
    shift
	java -jar $TARGET/benchmarks.jar $benchmark -foe true -gc true -rf csv -i $NB_ITERATION -wi $NB_ITERATION -f $NB_ITERATION -v SILENT -bm sample -tu ns -p db=$DB -t $THREAD -rff $TARGET/jmh.out $*
